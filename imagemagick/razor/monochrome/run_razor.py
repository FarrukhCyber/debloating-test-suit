...
def train():
    ...
    for fname in os.listdir('../train'):
        ...
        # -c
        cmd = BIN + ' ' + 'display' + ' ' + '-monochrome' + ' ' + fpath
        cmd = DRRUN + ' -c ' + CLIENT + ' -- ' + cmd
        execute(cmd)

def test():
    BIN = './magick.orig_temp/magick.orig.debloated'
...
def main():
    ...
    if sys.argv[1] == 'train':
        ...
        extend_debloat('magick', heuristic_level)

    elif sys.argv[1] == "get_test_traces":
        get_traces_for_test("logs", "magick")
